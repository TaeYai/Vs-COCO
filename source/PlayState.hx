...
class PlayState extends MusicBeatState
{
	...
	var parknightBG:FlxSprite;
	var parknightStars:FlxSprite;
	var parkBG:FlxSprite;
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'mango':
				dialogue = CoolUtil.coolTextFile(Paths.txt('mango/mangoDialogue'));
			case 'thorns':
				dialogue = CoolUtil.coolTextFile(Paths.txt('thorns/thornsDialogue'));
			case 'calm':
				dialogue = CoolUtil.coolTextFile(Paths.txt('calm/calmDialogue'));
			case 'joyful':
				dialogue = CoolUtil.coolTextFile(Paths.txt('joyful/joyfulPreDialogue'));
				dialogueEnd = CoolUtil.coolTextFile(Paths.txt('joyful/joyfulPostDialogue'));
			case 'calm-paster':
				dialogue = CoolUtil.coolTextFile(Paths.txt('calm-paster/calm-pasterDialogue'));
				dialogueEnd = CoolUtil.coolTextFile(Paths.txt('joyful/joyfulPostDialogue'));
		}
		...
		switch(stageCheck)
		{
			...
			case 'park-night':
			{
					...
					parknightStars.frames = stars;
					parknightStars.animation.addByPrefix('idle', 'stars');
					parknightStars.animation.play('idle');
					parknightStars.antialiasing = true;
					add(parknightStars);
	...
	/*	switch (curSong){
		...
		bar = new FlxSprite(0, 614); 
		bar.frames = Paths.getSparrowAtlas('bar');
		bar.animation.addByPrefix('white', 'WHITE', 24);
		bar.animation.addByPrefix('black', 'BLACK', 24);
		bar.antialiasing = true;
		if (FlxG.save.data.downscroll)
			bar.y = 18;
		bar.screenCenter(X);
		bar.scrollFactor.set();
		bar.animation.play('black');
		add(bar);
		...
		botPlayState.setFormat(Paths.font("vcr.ttf"), 42, FlxColor.WHITE, RIGHT);
	...
	function startSong():Void
	{
		...
		if (FlxG.save.data.songPosition)
		{
			...
			songPosBar.scrollFactor.set();
			songPosBar.createFilledBar(FlxColor.GRAY, 0xFF6B4436);
			add(songPosBar);

			var songName = new FlxText(songPosBG.x + (songPosBG.width / 2) - 20,songPosBG.y,0,SONG.song, 16);
			if (PlayStateChangeables.useDownscroll)
				songName.y -= 3;
			songName.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT);
			songName.scrollFactor.set();
			add(songName);
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			strumLineNotes.add(babyArrow);
					...
					smoke = new FlxSprite(600, 20); 
					smoke.frames = Paths.getSparrowAtlas('smoke_screen');
					smoke.animation.addByPrefix('idle', 'Smoke screen', 24);
					smoke.animation.addByPrefix('static', 'lmao', 24);
					smoke.antialiasing = true;
					smoke.cameras = [camHUD];
					smoke.animation.play('idle', true);
					smoke.alpha = 0;
					add(smoke);
	}
			...
			if (PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection && camFollow.x != boyfriend.getMidpoint().x - 100)
			{
				...
				switch (curStage)
				{
					case 'stage':
					{
						...
						{
							camFollow.x = boyfriend.getMidpoint().x - 120;
							camFollow.y = boyfriend.getMidpoint().y - 90;
						}
						...
						{
							camFollow.x = boyfriend.getMidpoint().x - 101;
							camFollow.y = boyfriend.getMidpoint().y - 110;
						}
						}
		...
		if (generatedMusic)
			{
				...
				var noteSpeed = FlxMath.roundDecimal(PlayStateChangeables.scrollSpeed == 1 ? SONG.speed : PlayStateChangeables.scrollSpeed, 2);

			if (PlayStateChangeables.useDownscroll)
{
    if (daNote.mustPress)
        daNote.y = (playerStrums.members[Math.floor(Math.abs(daNote.noteData))].y + 0.45 * (Conductor.songPosition - daNote.strumTime) * noteSpeed);
    else
        daNote.y = (strumLineNotes.members[Math.floor(Math.abs(daNote.noteData))].y + 0.45 * (Conductor.songPosition - daNote.strumTime) * noteSpeed);
    ...
    if(daNote.isSustainNote)
    {
        ...
        if(daNote.prevNote != null && daNote.animation.curAnim.name.endsWith('end')) {
            // Fixed by Ne_Eo
            var diffY = 0.45 * noteSpeed * stepCrochet;
        ...
        if(!PlayStateChangeables.botPlay)
        {
            ...
            daNote.clipRect = swagRect;
					...
					/*	switch (Math.abs(daNote.noteData))
						...
						if (iscoco)
							{
								coco.playAnim('sing' + dadsDir[daNote.noteData], true);
								coco.holdTimer = 0;
							}
						if (isdad)
							{
								...
								if (daNote.noteType == 2)
									{
										...
										health -= 10;
										vocals.volume = 0;
										FlxG.sound.play(Paths.sound('notesoundlmao'));
